            ...
            If ((GetTickCount() - CurrentUser.Queue(1).EventTick) >= BotVars.ChatDelay) Then
                ...
                For j = 1 To CurrentUser.Queue.Count
                    ...
                    Select Case (CurrentEvent.EventID)
                        ' ...
                        Case ID_USER
                            Call Event_UserInChannel(CurrentUser.Name, CurrentEvent.Flags, _
                                CurrentEvent.Statstring, CurrentEvent.Ping, CurrentEvent.GameID, _
                                    CurrentEvent.Clan, CurrentEvent.Statstring, CurrentEvent.IconCode, j)
                                
                        ' ...
                        Case ID_JOIN
                            Call Event_UserJoins(CurrentUser.Name, CurrentEvent.Flags, _
                                CurrentEvent.Statstring, CurrentEvent.Ping, CurrentEvent.GameID, _
                                    CurrentEvent.Clan, CurrentEvent.Statstring, CurrentEvent.IconCode, j)
