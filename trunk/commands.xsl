    ...
    With oCommandDoc
        If Not .OpenCommand("<xsl:value-of select="@name"/>", vbNullString) Then
            ...
            <xsl:if test="count(documentation/description) > 0">
                .Description = "<xsl:value-of select="replace(normalize-space(documentation/description), '&quot;', '&quot;&quot;')"/>"
            </xsl:if>
            
            <xsl:if test="count(documentation/specialnotes) > 0">
                .SpecialNotes = "<xsl:value-of select="replace(normalize-space(documentation/specialnotes), '&quot;', '&quot;&quot;')"/>"
            </xsl:if>
            ...
            <xsl:if test="count(access/flags/flag) > 0">
                .RequiredFlags = "<xsl:for-each select="access/flags/flag"><xsl:value-of select="text()"/></xsl:for-each>"
            </xsl:if>
            ...
            With oParameter
                ...
                <xsl:if test="count(match) > 0">
                    .MatchMessage = "<xsl:value-of select="replace(normalize-space(match[@message]), '&quot;', '&quot;&quot;')"/>"
                    .MatchCaseSensitive = <xsl:call-template name="getMatchCaseSenitive" />
                    <xsl:if test="count(error) > 0">
                        .MatchError = "#<xsl:value-of select="replace(normalize-space(error/text()), '&quot;', '&quot;&quot;')"/>"
                    </xsl:if>
                </xsl:if>

            <xsl:if test="count(access/rank) > 0">.RequiredRank = "<xsl:value-of select="access/rank"/>"</xsl:if>
