...
Private Sub cmd_<xsl:value-of select="@name"/>(ByRef oCommandDoc As clsCommandDocObj)
	...
	With oCommandDoc
		If Not .OpenCommand("<xsl:value-of select="@name"/>", vbNullString) Then
			...
			.Description = "<xsl:call-template name="replace-quotes"><xsl:with-param name="text" select="documentation/description"/></xsl:call-template>"
			</xsl:if>
			<xsl:if test="count(documentation/specialnotes) > 0">
			.SpecialNotes = "<xsl:call-template name="replace-quotes"><xsl:with-param name="text" select="documentation/specialnotes"/></xsl:call-template>"
			</xsl:if>
			
			.RequiredRank = <xsl:call-template name="getRank" />
			.RequiredFlags = <xsl:call-template name="getFlags" />
			...
			With oParameter
				...
				.MatchMessage = "<xsl:call-template name="replace-quotes"><xsl:with-param name="text" select="match[@message]"/></xsl:call-template>"
				.MatchCaseSensitive = <xsl:call-template name="getMatchCaseSenitive" />
				<xsl:if test="count(error) > 0">
				.MatchError = "<xsl:call-template name="replace-quotes"><xsl:with-param name="text" select="error/text()"/></xsl:call-template>"
				</xsl:if>
				</xsl:if>
